...
def _normalize_working_dir_bound_paths(data: object) -> object:
    ...
    This keeps COAPIS_WORKING_DIR effective even if user config files contain
    older hard-coded paths. Only rewrites known working-dir-bound keys.
    """
...
def get_available_channels() -> Tuple[str, ...]:
    ...
    coapis.channels), filtered by COAPIS_ENABLED_CHANNELS or
    COAPIS_DISABLED_CHANNELS when set.

    * COAPIS_ENABLED_CHANNELS — whitelist (only these channels are active).
    * COAPIS_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, COAPIS_ENABLED_CHANNELS takes precedence.
    * If neither is set, all discovered channels are returned.
    ...
    raw_enabled = EnvVarLoader.get_str("COAPIS_ENABLED_CHANNELS", "").strip()
    if raw_enabled:
...
def is_running_in_container() -> bool:
    ...
    Prefer env COAPIS_RUNNING_IN_CONTAINER (1/true/yes) at call time so
...
def get_config_path() -> Path:
    ...
    Returns SYSTEM_DIR/config.json (system-level config).
    Falls back to WORKING_DIR/config.json for backward compatibility.
    """
...
def is_coapis_running() -> bool:
    """Check if CoApis is currently running by checking API availability.

    Returns:
        True if CoApis is running, False otherwise
    """
    try:

...
"""`coapis doctor` — read-only checks.

`coapis doctor fix` — conservative repairs with backup.
"""
...
def _doctor_server_python_mismatch_note(
    ...
    if server_exe and doctor_exe:
        if not _same_python_executable(doctor_exe, server_exe):
            return (
                "This `coapis doctor` is not using the same Python "
                "executable as the running `coapis app` — diagnostics and "
    ...
    if doctor_env.strip() != server_env.strip():
        return (
            ...
            f"`coapis app` (doctor: {doctor_env!r}; server: "
            f"{server_env!r}). "
...
def _check_web_auth(base: str) -> tuple[bool, str]:
    ...
    if not has_registered_users():
        return (
            ...
            f"        1) Start `coapis app`, open {base}/ in a browser.\n"
            "        2) Complete registration (single user) on the login "
            "page.\n"
            "        For automation, set COAPIS_AUTH_USERNAME and "
            "COAPIS_AUTH_PASSWORD (legacy COAPIS_* names still work) — the "
...
async def _check_active_llm(
    ...
    if (
        ...
        return (
            False,
            "no active LLM slot — run `coapis models list` and configure "
            "an active model",
            [],
        )
    ...
    if deep and pid in ("coapis-local",):
        deep_notes = coapis_local_llm_deep_notes()
...
def run_doctor_checks(
    ...
    """Run read-only ``coapis doctor`` checks (no disk mutations)."""
    ...
    if config_ok:
        ...
        _doctor_fix_hint(
            ...
            "`coapis doctor fix --dry-run --help` and `--only`.",
        )
    ...
    if raw_cfg is not None:
        ...
        if unknown:
            ...
            _doctor_fix_hint(
                ...
                "(`coapis doctor` and `doctor fix` do not strip unknown keys "
                "yet).",
            )
    ...
    if config_ok:
        ...
        if ws_ok:
            ...
            _doctor_fix_hint(
                "Preview the plan (no writes): `coapis doctor fix --dry-run "
        ...
        if not ext_nonempty:
            click.echo(
                click.style("OK", fg="green")
                + " — no extension notes (register via coapis.doctor entry "
        ...
        click.echo(
            ...
            + _skipped_when_cfg_invalid
            + ". Fix the config file, then re-run `coapis doctor`.",
        )
        click.echo("\n=== Browser (browser_use / Playwright) ===")
    ...
    if wd_ok:
        ...
        _doctor_fix_hint(
            "Fix: set `COAPIS_WORKING_DIR` (or legacy `COAPIS_WORKING_DIR`) "
            "or run `coapis init`. "
            "Preview the plan (no writes): `coapis doctor fix --dry-run "
    ...
    if llm_ok:
        ...
        _doctor_fix_hint(
            "`coapis models list` / console model settings — not a "
            "filesystem fix.",
        )
    ...
    try:
        ...
        click.echo(
            f"Hint: start the server with `coapis app` (default {base}).",
            err=True,
        )
    else:
...
@click.option(
    ...
    help=(
        ...
        "uses --timeout) and, when the active model is coapis-local, "

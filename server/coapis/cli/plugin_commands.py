...
def _check_coapis_not_running():
    """Check if CoApis is not running, exit if it is."""
    from ..config.utils import is_coapis_running

    if is_coapis_running():
        click.echo(
            "❌ CoApis is currently running. Please stop it first:",
            err=True,
        )
        click.echo("   coapis shutdown", err=True)
        click.echo(
            "\n💡 Plugin operations are only allowed when CoApis is stopped.",
        )
        raise click.Abort()
...
def install(source: str, force: bool):
    ...
    Examples:
        coapis plugin install examples/plugins/idealab-provider
        coapis plugin install /path/to/plugin
        coapis plugin install https://example.com/plugin.zip
    """
    from ..config.utils import get_plugins_dir

    # Check if CoApis is running
    _check_coapis_not_running()
    ...
    click.echo("\n💡 Next steps:")
    click.echo("   1. Restart CoApis to load the plugin")
...
def uninstall(plugin_id: str):
    ...
    try:
        ...
        click.echo("💡 Restart CoApis to apply changes")
    except Exception as e:

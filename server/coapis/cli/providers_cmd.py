    ...
    try:
        ...
        click.echo(
            click.style(
                ...
                "Install with: pip install 'coapis[local]'",
                fg="red",
            ),
        )
...
def configure_llm_slot_interactive(*, use_defaults: bool = False) -> None:
    ...
    if not eligible:
        if use_defaults:
            click.echo(
                "No LLM provider configured. Run 'coapis models config' "
                "to configure later.",
            )
            return
        click.echo(
    ...
    if not model and use_defaults:
        click.echo(
            ...
            "Run 'coapis models config' to set one.",
        )
        return
    try:
    ...
    for defn in _all_provider_objects(manager):
        ...
        if defn.is_local:
            ...
            if all_models:
                ...
                click.echo("  Use 'coapis models download' to add models.")
        else:
...
def add_provider_cmd(
    ...
    click.echo(
        "  Run 'coapis models add-model' to add models, "
        "then 'coapis models config-key' to set the API key.",
    )
...
def download_cmd(
    ...
    \b
    Examples:
      coapis models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF
      coapis models download Qwen/Qwen2-0.5B-Instruct-GGUF --source modelscope
    """
    ...
    click.echo(
        ...
        "  coapis models set-llm  (select 'coapis-local' provider)",
    )
...
def list_local_cmd() -> None:
    ...
    if not models:
        ...
        click.echo("Use 'coapis models download <repo_id>' to download one.")
        return

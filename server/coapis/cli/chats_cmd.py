...
def chats_group() -> None:
    ...
    \b
    Common examples:
      coapis chats list                    # List all chats
      coapis chats list --user-id alice    # Filter by user
      coapis chats get <chat_id>           # View details
      coapis chats create --session-id s1 --user-id u1
      coapis chats delete <chat_id>        # Delete a chat
    """
...
def list_chats(
    ...
    Examples:
      coapis chats list
      coapis chats list --user-id alice
      coapis chats list --channel discord
      coapis chats list --user-id alice --channel discord
    """
...
def get_chat(
    ...
    \b
    CHAT_ID  Chat UUID, obtainable via `coapis chats list`.

    \b
    Examples:
      coapis chats get 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}
...
def create_chat(
    ...
    Inline creation examples:
      coapis chats create --session-id "discord:alice" \\
    ...
    JSON file creation example:
      coapis chats create -f chat.json
    """
    base_url = resolve_base_url(ctx, base_url)
...
def update_chat(
    ...
    \b
    Examples:
      coapis chats update <chat_id> --name "Renamed Chat"
    """
    base_url = resolve_base_url(ctx, base_url)
    headers = {"X-Agent-Id": agent_id}
    payload = {"name": name}
...
def delete_chat(
    ...
    \b
    Examples:
      coapis chats delete 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}

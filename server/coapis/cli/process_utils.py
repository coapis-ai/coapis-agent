...
def _matches_coapis_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a CoApis CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m coapis {subcommand}",
            f" coapis {subcommand}",
            f"__main__.py {subcommand}",
            f'coapis.exe" {subcommand}',
            f"coapis.exe {subcommand}",
        )
    )


def _is_coapis_service_command(command: str) -> bool:
    """Return whether the command line looks like a local CoApis app."""
    return _matches_coapis_cli_command(command, "app")


def _is_coapis_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a CoApis CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "coapis" or _matches_coapis_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local CoApis service."""
    if not host:
        return []

...
# Template for `coapis channels install <key>` stub (channel key substituted).
...
from coapis.app.channels.base import BaseChannel
from coapis.app.channels.schema import ChannelType
...
def configure_weixin(current_config: WeixinConfig) -> WeixinConfig:
    ...
    bot_token_file = click.prompt(
        "bot_token file path",
        default=(
            current_config.bot_token_file or "~/.coapis/weixin_bot_token"
        ),
        type=str,
    )
...
def configure_voice(
    ...
    welcome_greeting = click.prompt(
        ...
        or "Hi! This is CoApis. How can I help you?",
        type=str,
    )
...
@click.option(
    "--target-user",
    required=True,
    help=("Target user ID (REQUIRED, get from 'coapis chats list' query)"),
)
@click.option(
    "--target-session",
    required=True,
    help=("Target session ID (REQUIRED, get from 'coapis chats list' query)"),
)
@click.option(
    "--text",
    required=True,
...
def send_cmd(
    ...
    Complete Usage Flow:
      Step 1 - Query available sessions (REQUIRED):
        coapis chats list --agent-id my_bot --channel console
      ...
      Step 3 - Send message using queried parameters:
        coapis channels send --agent-id my_bot --channel console \\
    ...
    Examples with jq automation:
      ...
      SESSIONS=$(coapis chats list --agent-id bot --channel console)
      USER=$(echo "$SESSIONS" | jq -r '.[0].user_id')
      SESSION=$(echo "$SESSIONS" | jq -r '.[0].session_id')

      # Send message
      coapis channels send --agent-id bot --channel console \\
        --target-user "$USER" --target-session "$SESSION" \\
        --text "Automated notification"

    \b
    Prerequisites:
      1. MUST use 'coapis chats list' to get valid target-user and
         target-session

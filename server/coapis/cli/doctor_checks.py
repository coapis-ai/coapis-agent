...
"""Read-only diagnostics for `coapis doctor` (no config or disk mutations)."""
...
# Log file opened on app startup (see ``coapis.app._app`` lifespan).
APP_LOG_BASENAME = LOG_FILE_BASENAME

# Built-in local llama.cpp provider id.
_COAPIS_LOCAL_PROVIDER_IDS = frozenset({"coapis-local"})
...
def check_app_log_writable() -> tuple[bool, str]:
    ...
    if log_path.exists():
        ...
        return (
            ...
            "(required when starting `coapis app`)",
        )
...
def environment_summary_lines(
    ...
    *server_python_environment* describes the **HTTP API process** (running
    ``coapis app``), when ``GET /api/doctor/runtime`` returned
    ...
    lines = [
        ...
        f"coapis version: {__version__}",
        f"platform: {platform.system()} {platform.machine()}",
        f"doctor_python_environment: {doctor_env}",
    ]
    if server_python_environment is not None:
        lines.append(
            f"coapis_python_environment: {server_python_environment}",
        )
    else:
        lines.append(
            "coapis_python_environment: "
            + (server_python_note or "(unknown)"),
        )
    lines.append(f"working_dir: {WORKING_DIR}")
    wd_qp = os.getenv("COAPIS_WORKING_DIR")
    wd_legacy = os.getenv("COAPIS_WORKING_DIR")
    if wd_qp:
        lines.append(f"COAPIS_WORKING_DIR (env): {wd_qp}")
    elif wd_legacy:
        lines.append(f"COAPIS_WORKING_DIR (env, legacy): {wd_legacy}")
...
def legacy_single_agent_workspace_note(cfg: Config) -> str | None:
    ...
    When this applies, ``coapis app`` may run an automatic migration;
    ...
    return (
        ...
        "`coapis app` uses to trigger legacy → multi-agent workspace "
        "migration. Start `coapis app` once (or see docs / `coapis init`). "
...
def browser_automation_notes(cfg: Config | None) -> list[str]:
    ...
    use_default = (
        EnvVarLoader.get_str("COAPIS_BROWSER_USE_DEFAULT", "1")
        .strip()
        .lower()
    )
    if use_default in ("0", "false", "no", "off"):
        notes.append(
            "COAPIS_BROWSER_USE_DEFAULT is off — browser_use will not "
...
# --- CoApis checks (agent.json, channels, MCP, skills, providers) ---
...
def check_enabled_agents_load_agent_config(cfg: Config) -> tuple[bool, str]:
    """Dry-run :func:`~coapis.config.config.load_agent_config` for enabled.
    ...
    for agent_id, ref in cfg.agents.profiles.items():
        ...
        if not path.is_file():
            problems.append(
                ...
                "disk; ensure the file exists or use `coapis doctor fix` "
                "where applicable.",
            )
            continue
        try:
...
def coapis_local_llm_deep_notes() -> list[str]:
...
def console_static_diagnostic_notes() -> list[str]:
    ...
    from ..utils.console_static import (
        CONSOLE_STATIC_ENV,
        find_coapis_source_repo_root,
        resolve_console_static_dir,
    )

    notes: list[str] = []
    env_dir = EnvVarLoader.get_str("COAPIS_CONSOLE_STATIC_DIR", "").strip()
    if env_dir:
        notes.append(

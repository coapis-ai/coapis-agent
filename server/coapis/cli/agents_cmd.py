...
def _submit_background_task(
    ...
    try:
        ...
        click.echo(f"  coapis agents chat --background --task-id {task_id}")
...
def _check_task_status(
    ...
    try:
        ...
        if status == "finished":
            ...
            click.echo(
                f"  coapis agents chat --background --task-id {task_id}",
            )
...
def agents_group() -> None:
    ...
    \b
    Examples:
      coapis agents chat --from-agent bot_a --to-agent bot_b --text "..."
      coapis agents create --name "Research Bot" --agent-id research_bot
      coapis agents delete research_bot
      coapis agents list
    """
...
def list_agents(ctx: click.Context, base_url: Optional[str]) -> None:
    ...
    Examples:
      coapis agents list
      coapis agents list --base-url http://192.168.1.100:8088
...
def delete_cmd(
    ...
    \b
    AGENT_ID  Configured agent ID, obtainable via `coapis agents list`.

    \b
    Examples:
      coapis agents delete research
      coapis agents delete research --remove-workspace
      coapis agents delete research --yes
    """
...
def chat_cmd(
    ...
    \b
    Prerequisites:
      1. Use 'coapis agents list' to discover available agents
      2. Ensure target agent (--to-agent) is configured and running
      3. Use 'coapis chats list' to find existing sessions (optional)

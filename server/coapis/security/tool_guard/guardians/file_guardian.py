...
_SECRET_DIR_CURRENT_NAME = ".coapis.secret"
...
# ── Sensitive file name patterns (basename matching) ─────────────────
# These patterns match against the *filename* portion of a path, so
# even files outside the secret dir are caught if they have a sensitive name.
_SENSITIVE_FILE_NAMES: frozenset[str] = frozenset({
    ...
    "*.cert", "*.p10", "*.p7b",
    # Package manager lockfiles & configs (may leak tokens)
    ".npmrc", ".yarnrc", ".pypirc",
    ".gem/credentials",
    ".docker/config.json",
    ...
    # History files (may leak sensitive commands)
    ".bash_history", ".zsh_history", ".python_history",
    # Network & proxy configs
    ".netrc", ".htpasswd",
    # Other sensitive patterns
    ".master.key", ".secret.key", ".private.key",
    ".secrets",
})

# Glob patterns for sensitive file matching (basename check)
...
def _is_file_guard_enabled() -> bool:
    ...
    try:
        from coapis.config import load_config
...
class FilePathToolGuardian(BaseToolGuardian):
    ...
    def _is_sensitive(self, abs_path: str) -> bool:
        ...
        for dir_path in self._sensitive_dirs:
            ...
            if abs_path.startswith(trimmed + "\\"):
                return True
        # 3. Basename pattern match (sensitive file names)
        basename = Path(abs_path).name
        if basename in _SENSITIVE_FILE_NAMES:
            return True
        for pattern in _SENSITIVE_FILE_GLOBS:

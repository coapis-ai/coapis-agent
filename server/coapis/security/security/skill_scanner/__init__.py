...
"""
Skill security scanner for CoApis.
...
Quick start::
    ...
    from coapis.security.skill_scanner import SkillScanner
...
def _get_scan_mode(cfg: Any = None) -> str:
    ...
    Priority: env ``COAPIS_SKILL_SCAN_MODE`` > config > default ``warn``.
    """
    env = EnvVarLoader.get_str("COAPIS_SKILL_SCAN_MODE") or None
    if env is not None:
        val = env.lower().strip()
...
_WORKING_DIR_CURRENT_NAME = ".coapis"

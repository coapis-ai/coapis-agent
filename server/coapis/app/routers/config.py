...
async def _get_config_safe(request: Request):
    """Get config safely: try agent.config first.

    Returns the agent-specific config when the agent is resolved.
    Falls back to global config ONLY when no agent is resolved at all
    (legacy single-agent mode).

    For channel config isolation: each agent must return its OWN config,
    never fall back to global config for agent-scoped requests.
    """
    import logging
    ...
    try:
        ...
        logger.info(f"[_get_config_safe] agent resolved: {agent}")
        if agent is not None:
            logger.info(f"[_get_config_safe] agent.agent_id: {agent.agent_id}")
            logger.info(f"[_get_config_safe] agent.config type: {type(agent.config)}")
            logger.info(f"[_get_config_safe] agent.config has channels: {hasattr(agent.config, 'channels') if agent.config else 'N/A'}")
            
            # Agent resolved — always use its own config
            if hasattr(agent, "config") and agent.config is not None:
                # If agent.config is a dict (from MinimalWorkspace), convert to AgentProfileConfig
                if isinstance(agent.config, dict):
                    logger.info(f"[_get_config_safe] agent.config is dict, converting to AgentProfileConfig")
                    from ..config.config import AgentProfileConfig
                    try:
                        agent.config = AgentProfileConfig(**agent.config)
                        logger.info(f"[_get_config_safe] converted successfully, channels: {agent.config.channels}")
                    except Exception as e:
                        logger.error(f"[_get_config_safe] conversion failed: {e}")
                        return None, agent
                return agent.config, agent
            # Agent found but config is None — try loading from workspace
            from ..config.config import load_agent_config
            try:
                agent_cfg = load_agent_config(agent.agent_id)
                return agent_cfg, agent
            except Exception:
                pass
            # Last resort: return agent with None config (channels will be empty)
            return None, agent
    except Exception as e:
        logger.error(f"[_get_config_safe] exception: {e}")
        import traceback
        traceback.print_exc()

    # Fallback: no agent resolved (legacy single-agent mode)
    logger.info(f"[_get_config_safe] falling back to global config")
    cfg = load_config()
    return cfg, None
...
async def list_channels(request: Request) -> dict:
    ...
    Each agent has its OWN channel config — never falls back to global config.
    """
    agent_config, agent = await _get_config_safe(request)
    available = get_available_channels()

    # Get channel configs from agent's config (with fallback to empty)
    # IMPORTANT: agent_config may be None when agent is found but has no config yet
...
async def put_channels(
    ...
    agent_config, agent = await _get_config_safe(request)
    if agent is None:
        # Legacy single-agent mode: save directly to global config
        agent_config.channels = channels_config
        save_config(agent_config)
        return channels_config

    # Agent-scoped: save to agent's own config
    if agent.config is None:
        # Agent has no config yet, create a minimal one
        from ...config.config import AgentProfileConfig

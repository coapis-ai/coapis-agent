...
_DEFAULT_AGENT_DESCRIPTION = "Default CoApis agent"
...
def _migrate_workspace_items_from_source(
    ...
    Args:
        source_dir: Source directory (e.g., ~/.coapis or WORKING_DIR)
...
def _do_ensure_default_agent() -> None:
    ...
    if not agent_existed:
        ...
        if not config.agents.active_agent:
            config.agents.active_agent = "global_default"

        save_config(config)
        save_agent_config("global_default", template_result.agent_config)
        logger.info(
...
def _fallback_active_agent_id(config, exclude_id: str) -> str:
    ...
    for aid, ref in profiles.items():
        ...
        if getattr(ref, "enabled", True):
            return aid
    if "global_default" in profiles and exclude_id != "global_default":
...
def _apply_legacy_qa_disable_for_migration(config) -> None:
    ...
    if getattr(ref, "enabled", True):
        ...
        logger.info(
            ...
            "(new CoApis builtin QA slot was created)",
            legacy_id,
        )
...
def ensure_qa_agent_exists() -> None:
    ...
    ``coapis_source_index``), and built-in tools are restricted (see
...
def _do_ensure_qa_agent() -> None:
    ...
    Global agents (like QA agent) must use AGENTS_DIR, not WORKSPACES_DIR.
    WORKSPACES_DIR is strictly for user-owned workspaces.
    """
    from ..constant import AGENTS_DIR
...
# ═══════════════════════════════════════════════════════════
# Global templates initialization
# ═══════════════════════════════════════════════════════════

_FALLBACK_TEMPLATES: dict[str, str] = {
    "SOUL.md": (
        "# Agent Soul\n\n"
        "_This agent is becoming someone._\n\n"
        "## Core Principles\n\n"
        "**Help genuinely, don't perform.** Skip the "
        '"good question!" and "I\'m happy to help!" — just help. '
        "Actions speak louder than words.\n\n"
        "**Have your own opinions.** You can disagree, have preferences, "
        "find things interesting or boring. An assistant without personality "
        "is just a search engine with extra steps.\n\n"
        "**Try to figure it out first.** Read files. Check context. Search. "
        "See if there are skills or tools available. Ask only when stuck. "
        "Goal: come back with answers, not questions.\n"
    ),
    "MEMORY.md": (
        "# Memory\n\n"
        "## Important Decisions\n\n(No important decisions yet)\n\n"
        "## Tool Settings\n\n(No tool settings yet)\n\n"
        "## Notes\n\n(No notes yet)\n"
    ),
    "PROFILE.md": (
        "## Identity\n\n"
    ...
    "AGENTS.md": (
        "# AGENTS.md\n\n"
        "## Core Rules\n\n"
        "- **No duplicate execution**: Never call the same tool twice.\n"
        "- **No infinite loops**: Stop and report if stuck.\n\n"
        "## Security\n\n"
...
_TEMPLATE_FILES = list(_FALLBACK_TEMPLATES.keys())


def ensure_global_templates_exist() -> None:
    """Ensure system/templates/ exists with default template files.

    Creates the directory and any missing template files at startup.
    ...
    for filename, content in _FALLBACK_TEMPLATES.items():
        template_path = TEMPLATES_DIR / filename
        if not template_path.exists():
            template_path.write_text(content, encoding="utf-8")
            logger.info(f"Created global template: {filename}")


# ═══════════════════════════════════════════════════════════
# Global agent role migration
# ═══════════════════════════════════════════════════════════

# Known system agents and their default roles
_SYSTEM_AGENT_ROLES: dict[str, dict] = {
    "global_default": {"role": "template", "priority": 100},
    "global_qa_agent": {"role": "service", "priority": None},
}


def ensure_global_agent_roles() -> None:
    """Ensure global agents in agents/ have role and priority fields.

    Runs at startup; sets defaults for known system agents, skips
    agents that already have role configured.
    """
    from ..constant import AGENTS_DIR
    from ..config.utils import load_config, save_config
    ...
    for agent_dir in AGENTS_DIR.iterdir():
        ...
        # Apply defaults for known system agents
        defaults = _SYSTEM_AGENT_ROLES.get(agent_id, {})
        role = defaults.get("role", "template")
...
def ensure_layered_templates() -> None:
    """Ensure user-level and agent-level template directories exist.

    For existing users who were initialized before v0.6.1:
    - Creates user-level template files in their workspace root (SOUL.md, PROFILE.md, AGENTS.md)
    - Does NOT overwrite existing files (only creates missing ones)
    """
    from ..constant import WORKSPACES_DIR, TEMPLATES_DIR

    user_level_dir = TEMPLATES_DIR / "user_level"
    agent_level_dir = TEMPLATES_DIR / "agent_level"

    if not user_level_dir.exists() or not agent_level_dir.exists():
        logger.warning(
            f"Layered template dirs missing: user_level={user_level_dir.exists()}, "
            f"agent_level={agent_level_dir.exists()}. Skipping migration."
        )
        return
    ...
    # For each user workspace, ensure user-level files exist
    user_files = ["SOUL.md", "PROFILE.md", "AGENTS.md"]
    migrated = 0
    ...
    for user_dir in WORKSPACES_DIR.iterdir():
        ...
        if not agent_json.exists():
            continue  # Not a valid user workspace
        ...
        for filename in user_files:
            ...
            template = user_level_dir / filename
            if template.exists():
    ...
    if migrated:
        logger.info(f"ensure_layered_templates: migrated {migrated} files for existing users")
    else:
        logger.debug("ensure_layered_templates: no migration needed")

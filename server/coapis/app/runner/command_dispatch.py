...
"""Command dispatch: run command path without creating CoApisAgent.
...
def _resolve_user_role(user_id: str) -> str:
    """Resolve user role from user_id via user_system.

    Falls back to 'user' if lookup fails (e.g. auth disabled,
    external channel without user registration).
    """
    if not user_id:
        return "user"
    try:
...
async def run_command_path(  # pylint: disable=too-many-statements,too-many-branches  # noqa: E501
    ...
    if parsed is not None:
        ...
        user_role = _resolve_user_role(user_id)
    ...
    if _is_control_command(query):
        ...
        # Extract user_id and role from request

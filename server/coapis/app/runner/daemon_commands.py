...
class DaemonContext:
    ...
    session_id: str = ""
    # User role for permission gating (user/advanced/admin)
    role: str = "user"
...
class DaemonCommandHandlerMixin:
    ...
    async def handle_daemon_command(
        ...
        # Permission gating for high-risk daemon sub-commands
        _admin_only = {"restart", "reload-config", "logs"}
        if sub in _admin_only and context.role != "admin":
            return Msg(
                name="Friday",
                role="assistant",
                content=[
                    TextBlock(
                        ...
                        text=(
                            "❌ **权限不足**\n\n"
                            f"`/daemon {sub}` 需要 admin 角色。\n"
                            f"- 当前角色: `{context.role}`\n"
                            "- `/daemon status` 和 `/daemon version` "
                            "可由所有用户使用。"
                        ),
                    ),
                ],

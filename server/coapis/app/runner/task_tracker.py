...
class TaskTracker:
    ...
    async def register_external_task(self, run_key: str) -> None:
        ...
        This is used for tasks managed outside of CoApis's own streaming
    ...
    async def attach_or_start(
        ...
        Args:
            on_complete: Optional async callback invoked after the producer
                finishes. Receives ``(run_key, payload)``. Useful for
                post-stream persistence that must happen regardless of
                whether the SSE client is still connected.
        """
        async with self._lock:
            state = self._runs.get(run_key)
            if state is not None and not state.task.done() and not state.stopped:
                q: asyncio.Queue = asyncio.Queue()
                for sse in state.buffer:
                    q.put_nowait(sse)
                state.queues.append(q)
            ...
            async def _producer() -> None:
                ...
                try:
                    ...
                    logger.info(f"_producer done: run_key={run_key}, events={event_count}")
                    # ── Invoke completion callback ──
                    if on_complete is not None:
                        try:
                            await on_complete(run_key, payload)
                        except Exception:
                            logger.exception(
                                "on_complete callback error for run_key=%s",
                                run_key,
                            )
                    tracker = tracker_ref()
                    if tracker is not None:
                        async with tracker.lock:
                            for q in run.queues:
                                q.put_nowait(_SENTINEL)
                            # Only remove from _runs if this run is still
                            # the active one (not replaced by a new run
                            # after /stop).
            ...
            logger.info(f"Created _producer task for run_key={run_key}")

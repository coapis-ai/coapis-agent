...
class ModelCommandHandler(BaseControlCommandHandler):
    ...
    async def handle(self, context: ControlContext) -> str:
        ...
        # Read-only sub-commands: no permission check needed
        if not args_str:
        ...
        # Write sub-commands: require advanced+ role
        if context.role not in ("admin", "advanced"):
            return (
                "❌ **权限不足**\n\n"
                f"切换/重置模型需要 advanced 或 admin 角色。\n"
                f"- 当前角色: `{context.role}`\n"
                f"- `/model list` 和 `/model info` 可由所有用户使用。"
            )

        if args_str.lower() == "reset":

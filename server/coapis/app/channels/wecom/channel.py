...
# Global instance cache: bot_id -> WecomChannel
# Prevents multiple agents from creating WebSocket connections
# to the same bot_id, which causes connection conflicts.
_WECOM_INSTANCE_CACHE: dict = {}
# P1: Track which process callback owns each bot_id (bot_id -> id(process))
_WECOM_BOT_OWNER: dict = {}
...
def _split_markdown_blocks(text: str) -> List[str]:
    """Split text into logical blocks by markdown headings and separators.

    Split points:
      - Lines starting with ## (h2 headings — section boundaries)
      - Horizontal rules: ---, ***, ___ (alone on a line) — merged into next block

    Returns:
        List of text blocks. Empty blocks are stripped.
        If no split points found, returns [text] as-is.
    """
    ...
    for line in lines:
        ...
        is_hr = bool(_hr_re.match(line))

        if is_heading and current:
            # Flush current block before starting a new ## section
            block = "\n".join(current).strip()
            if block:
                blocks.append(block)
            current = [line]
        elif is_hr:
            # Horizontal rule: end current block but don't start new one
            # (the HR content gets discarded; next heading/text starts fresh)
            block = "\n".join(current).strip()
            if block:
                blocks.append(block)
            current = []
...
class WecomChannel(BaseChannel):
    ...
    def from_config(
        ...
        # Global instance cache: reuse existing WecomChannel for same bot_id
        if bot_id and bot_id in _WECOM_INSTANCE_CACHE:
            existing = _WECOM_INSTANCE_CACHE[bot_id]
            # P0: Detect bot_id conflict — different process callback = different agent
            owner_id = _WECOM_BOT_OWNER.get(bot_id)
            current_id = id(process)
            if owner_id is not None and owner_id != current_id:
                logger.error(
                    "BOT_ID CONFLICT: bot_id=%s is already bound to another agent! "
                    "Messages will be routed to the WRONG agent. "
                    "Each agent MUST use a unique bot_id. "
                    "Rejecting second registration.",
                    bot_id[:20],
                )
                raise ChannelError(
                    channel_name="wecom",
                    message=(
                        f"bot_id '{bot_id[:10]}...' is already used by another agent. "
                        f"Each agent must use a unique bot_id. "
                        f"Please change the bot_id in this agent's wecom config."
                    ),
                )
            logger.info(
                "WecomChannel.from_config: reusing cached instance for bot_id=%s",
                bot_id[:20],
            )
            return existing
        ...
        if bot_id:
            _WECOM_INSTANCE_CACHE[bot_id] = instance
            _WECOM_BOT_OWNER[bot_id] = id(process)
            logger.info(
                "WecomChannel.from_config: registered bot_id=%s owner_process=%s",
                bot_id[:20],
                id(process),
            )
...
# ------------------------------------------------------------------
    ...
    async def _on_message(self, frame: Any) -> None:
        ...
        try:
            ...
            if not allowed:
                ...
                # 返回用户ID，方便用户自行添加到白名单
                deny_msg = error_msg or "Access denied."
                if "不在允许" in deny_msg or "not allow" in deny_msg.lower() or deny_msg == "Access denied.":
                    deny_msg = f"{deny_msg}\n\n你的ID: {sender_id}\n请将此ID添加到白名单中。"
    ...
    async def _before_consume_process(
        ...
        # Store on request for later pickup by on_streaming_start
    ...
    def _get_streaming_sids(
        ...
        """Return per-stream_type sid mapping, lazily initialized."""
    ...
    async def send_content_parts(  # pylint: disable=too-many-locals
        ...
        # If streaming already delivered the text via on_streaming_delta/end,
        # skip text sending to avoid duplication. Only send media parts.
        if self._streaming_text_sent:
            self._streaming_text_sent = False  # Reset for next cycle
    ...
    async def start(self) -> None:
        ...
        # Prevent duplicate start() calls on the same instance
        if getattr(self, '_started', False):
            logger.info("wecom channel already started (bot_id=%s), skipping duplicate start()", self.bot_id[:10] if self.bot_id else "unknown")
            return
        self._started = True
        ...
        # FIX: SDK client.py has a bug — its lambda
        #   lambda frame: self._message_handler.handle_frame(frame)
        # omits the required "emitter" argument.  Call handle_frame
        # directly with both frame and emitter so MessageHandler
        # can properly dispatch to on("message") / on("event.*").
        _message_handler = self._client._message_handler
    ...
    # ------------------------------------------------------------------
# Bot ID registry helpers
# ------------------------------------------------------------------

def get_wecom_bot_registry() -> dict:
    """Return a snapshot of the bot_id → owner mapping.
    
    Useful for startup validation and debugging.
    Returns dict: {bot_id: {"owner_process_id": int, "instance": WecomChannel}}
    """
    result = {}
    for bot_id, owner_id in _WECOM_BOT_OWNER.items():
        instance = _WECOM_INSTANCE_CACHE.get(bot_id)
        result[bot_id] = {
            "owner_process_id": owner_id,
            "has_instance": instance is not None,
            "bot_prefix": getattr(instance, "bot_prefix", "") if instance else "",
        }
    return result

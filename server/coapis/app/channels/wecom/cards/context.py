...
async def send_stream_detail(
    ...
    """Stream ``body_text`` to the user, reusing the active proc."""
    client = getattr(channel, "_client", None)
    if not client or not frame:
        return
    sids = channel._get_streaming_sids(send_meta)
    stream_id = sids.get("message", "")
    if not stream_id:
        from aibot import generate_req_id
        stream_id = generate_req_id("stream")
        sids["message"] = stream_id
    try:
        ...
        logger.warning(
            "wecom send_stream_detail failed stream_id=%s",
            stream_id[:20] if stream_id else "?",
            exc_info=True,
        )

...
class QQChannel(BaseChannel):
    ...
    async def _send_deny_reply(
        ...
        """Send deny message back to user via QQ API."""
        try:
            ...
            await self._send_text_with_fallback(
                message_type=message_type,
                sender_id=sender_id,
                channel_id=self._config.get("appid", ""),
            ...
            logger.debug("qq deny reply failed for %s", sender_id)
    ...
    def _handle_msg_event(
        ...
        # --- Allowlist check (consistent with other channels) ---
        is_group = bool(
            meta.get("message_type") == "group"
        ...
        if not allowed:
            logger.info(
                "qq allowlist blocked: sender=%s is_group=%s",
                sender,
                is_group,
            )
            # 返回用户ID，方便用户自行添加到白名单
            ...
            if "不在允许" in deny_msg or "not allow" in deny_msg.lower() or deny_msg == "Access denied." or not deny_msg:
                deny_msg = f"{deny_msg}\n\n你的ID: {sender}\n请将此ID添加到白名单中。"
            # QQ uses fire-and-forget ACK pattern; schedule deny reply
            loop = self._loop
            if loop and loop.is_running() and self._http:
                coro = self._send_deny_reply(spec.message_type, sender, meta, deny_msg)
                try:
                    asyncio.run_coroutine_threadsafe(coro, loop)
                except Exception:
                    logger.debug("qq deny message failed to schedule for %s", sender)
            return

    ...
    async def _on_message(
        ...
        try:
            ...
            if not allowed:
                ...
                if "不在允许" in deny_msg or "not allow" in deny_msg.lower() or deny_msg == "Access denied." or not deny_msg:
                    deny_msg = f"{deny_msg}\n\n你的ID: {from_user_id}\n请将此ID添加到白名单中。"
                if deny_msg and context_token:
                    if self._loop:
            ...
            # --- Session rotation: intercept /new and /clear commands ---
            is_session_cmd = self.is_new_session_command(text)

            if is_session_cmd:
                # Use OLD session_id so the runner can summarize/clear
                # the old conversation memory properly.
                ...
                logger.info(
                    "weixin /new: from=%s old_session=%s",
                    (from_user_id or "")[:20],
                    session_id[:30] if session_id else "",
                )
                if self._enqueue is not None:
                    self._enqueue(native)

                # Immediately rotate to a new session for subsequent messages
                new_session_id = self.rotate_user_session(
                    from_user_id, channel_meta=meta,
                )
                logger.info(
                    "weixin /new: rotated to new_session=%s for user=%s",
                    new_session_id[:30] if new_session_id else "",
                    (from_user_id or "")[:20],
                )
                return  # Don't send /new text to the agent

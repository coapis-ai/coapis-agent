...
def create_mcp_context_token(username: str, agent_id: str) -> str:
    """Create HMAC-signed internal context token for MCP Gateway calls.
    
    This token represents the current user's execution context when calling
    external MCP servers through the gateway. It includes:
    - username: The executing user
    - agent_id: The workspace/agent ID (e.g., 'user:{username}')
    - scope: mcp_execution
    
    Args:
        username: Executing username
        agent_id: Agent/workspace identifier
        
    Returns:
        HMAC-signed token string
    """
    # Short-lived token for MCP context (5 minutes)
    expiry_seconds = 300
...
def verify_mcp_context_token(token: str) -> Optional[dict]:
    """Verify MCP context token, return payload if valid, None otherwise."""
    try:
        parts = token.split(".", 1)
        if len(parts) != 2:
            return None
        ...
        # Verify scope is mcp_execution
        if payload.get("scope") != "mcp_execution":
            return None

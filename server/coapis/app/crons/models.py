...
class CronExecutionRecord(BaseModel):
    """Per-job execution history record, inspired by CoApis CronExecutionRecord.

    Stores run_at, status, duration, error, trigger_type for each execution.
    Kept in-memory with a bounded deque per job_id for debugging and monitoring.
    """
    ...
    trigger_type: Literal["cron", "manual", "startup"] = "cron"
    task_type: TaskType = "agent"

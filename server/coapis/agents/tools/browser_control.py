...
def _resolve_output_path(path: str) -> str:
    """Resolve relative output paths under workspace_dir/files/browser/."""
    if Path(path).is_absolute():
        return path
    base_dir = (get_current_workspace_dir() or WORKING_DIR) / "files" / "browser"
...
def _browser_output_dir(state: dict, name: str) -> Path:
    ...
    output_dir = base_dir / "files" / "browser" / name
...
async def _configure_download_behavior(state: dict) -> None:
    ...
    try:
        ...
        logger.debug("Failed to configure CDP download behavior: %s", e)
    finally:
        if cdp is not None:
            try:
                await cdp.close()
...
async def _download_context_url(
    ...
    """Download a URL via browser context request (preserves cookies/session)."""
...
def _direct_url_download_rejected_response(
    ...
    """Build error response for rejected direct URL downloads."""
    payload = {
        "ok": False,
        "error": error.reason,
        "hint": (
...
async def _action_file_download(
    state: dict,
    page_id: str,
    file_path: str,
    ref: str = "",
    ...
    try:
            ...
            if not content_type:
                return _tool_response(
                    json.dumps(
                        {
                            "ok": False,
                            "error": f"File download failed: browser-context request returned HTTP {status}",
                            "page_id": page_id,
                            "url": source_url,
                            "status": status,
                            "file_path": resolved,
        ...
        # Ref-based download: click element and wait for download event
        if ref:
            locator = _get_locator_by_ref(state, page, ref, frame_selector="")
            if locator is None:
                return _tool_response(
                    json.dumps(
                        {"ok": False, "error": f"Ref '{ref}' not found"},
                        ensure_ascii=False,
                        indent=2,
                    ),
                )
            ...
            async def on_download(d):
                try:
                    ...
                    safe_name = _safe_download_filename(suggested, "download")
                    dest_dir = _browser_output_dir(state, "downloads")
                    dest = dest_dir / safe_name
                    await d.save_as(str(dest))
                    downloaded_path[0] = str(dest)
                except Exception as e:
                    logger.warning("Download save failed: %s", e)
                finally:
                    download_event.set()
            ...
            try:
                ...
                await asyncio.wait_for(download_event.wait(), timeout=timeout_ms / 1000.0)
            except asyncio.TimeoutError:
                return _tool_response(
                    json.dumps(
                        {
                            "ok": False,
                            "error": f"Download timed out after {wait_time:.1f}s",
                            "page_id": page_id,
                            "ref": ref,
                        },
                        ensure_ascii=False,
            ...
            return _tool_response(
                json.dumps(
                    {"ok": False, "error": "Download event triggered but no file saved"},
                    ensure_ascii=False,
                    indent=2,
                ),
            )

        # No ref and no url: error
        return _tool_response(
            json.dumps(
                {"ok": False, "error": "file_download requires either 'ref' or 'url' parameter"},
                ensure_ascii=False,
                indent=2,
            ),
        )
        ...
        logger.error("file_download error: %s", e, exc_info=True)
        return _tool_response(
...
async def _action_batch(
    ...
    """Execute multiple sub-actions sequentially."""
    if not actions_json:
        return _tool_response(
            json.dumps({"ok": False, "error": "actions_json is required"}, ensure_ascii=False, indent=2),
        )
    try:
        actions = json.loads(actions_json)
    except json.JSONDecodeError as e:
        return _tool_response(
            json.dumps({"ok": False, "error": f"Invalid JSON: {e}"}, ensure_ascii=False, indent=2),
        )
    if not isinstance(actions, list):
        return _tool_response(
            json.dumps({"ok": False, "error": "actions_json must be a JSON array"}, ensure_ascii=False, indent=2),
        )

    results = []
    for i, action_obj in enumerate(actions):
        if not isinstance(action_obj, dict) or "action" not in action_obj:
            results.append({"index": i, "ok": False, "error": "Missing 'action' key"})
            continue
        try:
            # Reinvoke browser_use with sub-action params
            sub_result = await browser_use(
                action=action_obj["action"],
                url=action_obj.get("url", ""),
                page_id=action_obj.get("page_id", page_id),
                selector=action_obj.get("selector", ""),
                text=action_obj.get("text", ""),
                code=action_obj.get("code", ""),
                path=action_obj.get("path", ""),
                wait=action_obj.get("wait", 0),
                full_page=action_obj.get("full_page", False),
                width=action_obj.get("width", 0),
                height=action_obj.get("height", 0),
                level=action_obj.get("level", "info"),
                filename=action_obj.get("filename", ""),
                accept=action_obj.get("accept", False),
                prompt_text=action_obj.get("prompt_text", ""),
                ref=action_obj.get("ref", ""),
                element=action_obj.get("element", ""),
                paths_json=action_obj.get("paths_json", ""),
                fields_json=action_obj.get("fields_json", ""),
                key=action_obj.get("key", ""),
                submit=action_obj.get("submit", False),
                slowly=action_obj.get("slowly", False),
                include_static=action_obj.get("include_static", False),
                screenshot_type=action_obj.get("screenshot_type", "png"),
                snapshot_filename=action_obj.get("snapshot_filename", ""),
                double_click=action_obj.get("double_click", False),
                button=action_obj.get("button", "left"),
                modifiers_json=action_obj.get("modifiers_json", ""),
                start_ref=action_obj.get("start_ref", ""),
                end_ref=action_obj.get("end_ref", ""),
                start_selector=action_obj.get("start_selector", ""),
                end_selector=action_obj.get("end_selector", ""),
                start_element=action_obj.get("start_element", ""),
                end_element=action_obj.get("end_element", ""),
                values_json=action_obj.get("values_json", ""),
                tab_action=action_obj.get("tab_action", ""),
                index=action_obj.get("index", -1),
                wait_time=action_obj.get("wait_time", 0),
                text_gone=action_obj.get("text_gone", ""),
                frame_selector=action_obj.get("frame_selector", ""),
                actions_json=action_obj.get("actions_json", ""),
                cdp_url=action_obj.get("cdp_url", ""),
                port=action_obj.get("port", 0),
                port_min=action_obj.get("port_min", 0),
                port_max=action_obj.get("port_max", 0),
                file_path=action_obj.get("file_path", ""),
            )
            # Parse result text
            result_text = sub_result.text if hasattr(sub_result, "text") else str(sub_result)
            parsed = json.loads(result_text) if result_text.strip() else {}
            results.append({"index": i, "ok": parsed.get("ok", True), "result": parsed})
        except Exception as e:
            results.append({"index": i, "ok": False, "error": str(e)})

    all_ok = all(r.get("ok", False) for r in results)
    return _tool_response(
        json.dumps({"ok": all_ok, "results": results}, ensure_ascii=False, indent=2),
    )


def stop_all_browsers() -> None:
    """Stop all browser instances across all workspaces."""
    global _workspace_states
    for wid, ws_state in list(_workspace_states.items()):
        try:
            _reset_browser_state(ws_state)
        except Exception as e:
            logger.warning("Failed to stop browser for workspace %s: %s", wid, e)
    _workspace_states.clear()
...
def _ensure_playwright_async():
    ...
    try:
        ...
        raise ImportError(
            "Playwright not installed. Use the same Python that runs CoApis (e.g. "
...
@register_tool(
    ...
    tags=['browser', 'web'],
    dependencies=[
        {"name": "playwright", "manager": "pip", "required": False, "reason": "浏览器自动化"},
    ],
)
async def browser_use(  # pylint: disable=R0911,R0912
    ...
    Args:
        action (str):
            ...
            file_upload, file_download, fill_form, install, press_key, run_code,
            drag, hover, select_option, tabs, wait_for, pdf, close, batch,
            cookies_get, cookies_set, cookies_clear, connect_cdp, list_cdp_targets,
            clear_browser_cache.
        ...
        cdp_port (int):
            ...
            CoApis chooses a free local port automatically for managed CDP.
        private_mode (bool):
            ...
            via CDP. Default False. By default, CoApis prefers managed CDP for
    ...
    try:
        ...
        if action == "file_download":
            return await _action_file_download(
                state, page_id, file_path, ref, url, wait_time,
            )
        if action == "batch":

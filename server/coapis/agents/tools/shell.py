...
def _set_child_resource_limits() -> None:
    """Set resource limits for child shell processes (preexec_fn).

    Enforces per-process limits to prevent a single user's command from
    exhausting host resources.  Only effective on Linux/macOS.

    Limits:
    - CPU: 60s
    - Address space (RSS): 512 MB
    - File size: 200 MB
    - Open file descriptors: 256
    - Child processes: 100
    - Wall clock (no kernel enforcement, but RLIMIT_RSS helps)
    """
    try:
        # CPU time: 60 seconds soft/hard
        ...
        # Some limits may not be settable (e.g., raised above hard limit).
        # Silently ignore — best-effort enforcement.
        pass

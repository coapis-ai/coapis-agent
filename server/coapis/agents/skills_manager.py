...
_REQUIREMENTS_METADATA_NAMESPACES = ("openclaw", "coapis", "clawdbot")
...
def get_builtin_skills_dir() -> Path:
    ...
    # Note: skills are in agent/skills/ (singular), not agents/skills/ (plural)
    return Path(__file__).parent.parent / "agent" / "skills"
...
def _skill_config_env_var_name(skill_name: str) -> str:
    ...
    return (
        f"COAPIS_SKILL_CONFIG_{''.join(normalized).strip('_') or 'DEFAULT'}"
    )
...
def apply_skill_config_env_overrides(
    ...
    ``COAPIS_SKILL_CONFIG_<SKILL_NAME>`` (JSON string).
    """
    ...
    def _update(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for skill_name in list(skills):
            if skill_name not in discovered:
                # Preserve global-default entries (they live in skill pool,
                # not in the workspace directory — don't remove them).
                if skills[skill_name].get("source") == "global":
                    continue
...
def _get_user_workspace_dir(agent_workspace_dir: Path) -> Path:
    """Get the user-level workspace dir from an agent-level workspace dir."""
    parts = agent_workspace_dir.parts
    for i, part in enumerate(parts):
        if part == "agents":
            return Path(*parts[:i])
    return agent_workspace_dir


def _resolve_agent_manifest(workspace_dir: Path) -> dict[str, Any]:
    """Read agent-level manifest from agents/{agent_id}/skills_manifest.json."""
    manifest_path = workspace_dir / "skills_manifest.json"
    if manifest_path.exists():
        return _read_json_mtime_cached(manifest_path, _default_workspace_manifest())
...
def _merge_manifests(
    ...
    for skill_name, entry in agent_manifest.get("skills", {}).items():
        if entry.get("enabled") is not None:
            merged["skills"][skill_name] = dict(entry)
            merged["skills"][skill_name]["source"] = "agent"
...
def resolve_effective_skills(
    ...
    if _is_agent_workspace(workspace_dir):
        ...
        user_manifest = read_skill_manifest(user_ws)
        agent_manifest = _resolve_agent_manifest(workspace_dir)
    else:
    ...
    # Determine skill search paths: agent skills + user skills + pool
    ...
    for skill_name, entry in sorted(manifest.get("skills", {}).items()):
        ...
        if "all" not in channels and channel_name not in channels:
            continue
        # Check if skill dir exists in any search path
        skill_found = any(
            (search_dir / skill_name).exists() for search_dir in search_dirs
        )
        if skill_found:
...
def _extract_emoji_from_metadata(metadata: Any) -> str:
    """Extract emoji from metadata.coapis.emoji."""
    if not isinstance(metadata, dict):
        return ""
    coapis = metadata.get("coapis")
    if isinstance(coapis, dict):
        return str(coapis.get("emoji", "") or "")
    return ""
...
def _extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="coapis_skill_upload_"))
...
def _staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"coapis_skill_stage_{skill_name}_"),
    )
...
class SkillService:
    ...
    def _save_skill_in_place(
        ...
        *,
        ...
        old_category = old_entry.get("category") or ""
        category_changed = category is not None and category != old_category
        if not content_changed and not category_changed and new_config == (
        ...
        if content_changed:
            ...
            try:
                from coapis.skill_evolution.version_manager import (
                    get_version, bump_version, set_version_in_content,
                    archive_skill_dir_version,
                )
                old_version = get_version(old_md) if old_md else "1.0"
                new_version = bump_version(old_version)
                saved_version = new_version
                # 存档旧版本目录（如果存在）
                if skill_dir.exists() and old_md:
                    archive_skill_dir_version(skill_name, old_version, skill_dir)
                # 更新 content 中的版本号
                content = set_version_in_content(content, new_version)
            except Exception as exc:
                logger.debug("Version management skipped: %s", exc)
        ...
        def _edit(payload: dict[str, Any]) -> None:
            ...
            if existing_tags is not None:
                next_entry["tags"] = existing_tags
            cat = category if category is not None else current_entry.get("category")
            if cat is not None:
                next_entry["category"] = cat
    ...
    def _save_skill_as_rename(
        ...
        *,
        ...
        try:
            ...
            old_md_path = old_dir / "SKILL.md"
            old_md = old_md_path.read_text(encoding="utf-8") if old_md_path.exists() else ""
            old_version = get_version(old_md) if old_md else "1.0"
            new_version = bump_version(old_version)
            saved_version = new_version
            if old_dir.exists() and old_md:
                archive_skill_dir_version(skill_name, old_version, old_dir)
            content = set_version_in_content(content, new_version)
        except Exception as exc:
            logger.debug("Version management (rename) skipped: %s", exc)
    ...
    def enable_skill(
        ...
        # Enabling a skill flips manifest state. For on-disk workspace skills
        # we run a security scan first; for global/pool skills (source=global)
        # we skip the scan since the skill lives in the shared pool.
        ...
        # Read manifest to check if this is a global/pool skill
        manifest = _read_json(manifest_path, _default_workspace_manifest())
        entry = manifest.get("skills", {}).get(skill_name)
        is_global = entry is not None and entry.get("source") == "global"

        if not is_global:
            # Non-global skill: must exist on disk and pass scan
    ...
    def delete_skill(self, name: str) -> bool:
        ...
        skill_dir = get_workspace_skills_dir(self.workspace_dir) / skill_name
        # Double-check: resolved path must be inside workspace skills dir
        skills_base = get_workspace_skills_dir(self.workspace_dir)
        if skill_dir.resolve().is_relative_to(skills_base.resolve()):
            if skill_dir.exists():
    ...
    def load_skill_file(
        ...
        skill_name = _normalize_skill_dir_name(skill_name)
        ...
        # Final boundary check: resolved path must stay inside skill dir
...
class SkillPoolService:
    ...
    def delete_skill(self, name: str) -> bool:
        ...
        # Double-check: resolved path must be inside pool dir
        pool_base = get_skill_pool_dir()
        if skill_dir.resolve().is_relative_to(pool_base.resolve()):
            if skill_dir.exists():
                shutil.rmtree(skill_dir)

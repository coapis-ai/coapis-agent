...
def build_bootstrap_guidance(
    ...
    if language == "zh":
        return (
            ...
            "这是首次对话。请遵循以下规则：\n"
            "\n"
            "1. 如果用户问了具体问题，请先回答用户的问题。\n"
            "2. 回答完毕后，友好地询问用户如何称呼。\n"
            "3. 只进行简单对话，不要调用任何工具或创建文件。\n"
            "4. 保持简洁，不要过度探索。\n"
            "\n"
        )
    ...
    return (
        "# Bootstrap Mode\n"
        "\n"
        "This is the first conversation. Please follow these rules:\n"
        "\n"
        "1. If the user asked a specific question, answer it first.\n"
        "2. After answering, politely ask how the user would like to be addressed.\n"
        "3. Keep the conversation simple and do NOT call any tools or create files.\n"
        "4. Keep it concise and avoid over-exploring.\n"
        "\n"
    )

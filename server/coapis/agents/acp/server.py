...
"""CoApis ACP Agent server.

Exposes CoApis as an ACP-compliant agent that external clients
...
def _msg_to_updates(  # pylint: disable=too-many-branches
    ...
    """Convert a CoApis Msg into ACP session update(s).
...
class CoApisACPAgent(Agent):
    ...
    async def _ensure_workspace(self) -> Any:
        ...
        logger.info(
            "CoApis ACP Agent workspace started: agent_id=%s workspace=%s",
            agent_id,
            workspace_dir,
        )
        return runner
...
async def run_coapis_agent(
    agent_id: str | None = None,
    workspace_dir: Path | None = None,
) -> None:
    """Entry point: run CoApis as an ACP agent over stdio."""
    agent = CoApisACPAgent(
        agent_id=agent_id,

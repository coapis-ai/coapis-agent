...
class ProviderManager:  # pylint: disable=too-many-public-methods
    ...
    def _add_builtin(self, provider: Provider):
        """Add a built-in provider, creating a copy to avoid reference aliasing.

        This prevents _init_from_storage from modifying the module-level
        constant when it overwrites fields from disk.
        """
        # Create a shallow copy to break reference to module-level constant
        provider_copy = provider.model_copy()
    ...
    def start_local_model_resume(self, local_manager) -> None:
        ...
        task = asyncio.create_task(
            ...
            name="coapis-local-model-resume",
        )
    ...
    def _init_from_storage(self):
        ...
        for builtin in self.builtin_providers.values():
            ...
            if provider:
                ...
                # and the stored value is non-empty (prevents corrupted JSON
                # from overwriting valid defaults)
                if not builtin.freeze_url and provider.base_url:

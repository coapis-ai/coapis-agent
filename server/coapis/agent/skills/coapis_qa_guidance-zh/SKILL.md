...
当用户询问 **CoApis 的安装、初始化、环境配置、依赖要求、常见配置项** 时，使用本 skill。
...
执行以下脚本逻辑来获取变量 $COAPIS_ROOT：

```bash
# 获取二进制绝对路径
COP_PATH=$(which coapis 2>/dev/null || whereis coapis | awk '{print $2}')

# 逻辑推导：如果路径包含 .coapis/bin/coapis，则根目录在其上三层
# 例如：/path/to/CoApis/.coapis/bin/coapis -> /path/to/CoApis
...
如果还是找不到文档，搜索 coapis 安装路径下的可用文档内容：

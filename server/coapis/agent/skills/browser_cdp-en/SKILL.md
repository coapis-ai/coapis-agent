...
- This **attaches to an external browser**, not a new process launched by CoApis
...
### 1. Managed CDP launched by CoApis
...
These browsers are launched and owned by CoApis. `stop` will:
...
These browsers are not launched by CoApis. `stop` will only:

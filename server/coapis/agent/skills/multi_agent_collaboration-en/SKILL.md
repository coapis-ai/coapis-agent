...
description: Use this skill when another agent's expertise or context is needed, or when the user explicitly asks to involve another agent. First list agents, then use coapis agents chat for two-way communication with replies.
metadata:
...
```bash
coapis agents chat --background --task-id <task_id>
```
...
|-----------|-------------|---------|
| Simple quick query | Real-time mode | `coapis agents chat` |
| Complex task (data analysis, batch processing, etc.) | Background mode | `coapis agents chat --background` |
...
2. coapis agents list
3. coapis agents chat to start a conversation
...
2. coapis agents list
3. coapis agents chat --background to submit the task
...
`coapis agents chat` must include all of the following:
- `--from-agent`
- `--to-agent`
- `--text`
...
Check status with:
  coapis agents chat --background --task-id 20802ea3-...
```
...
sleep 30 && coapis agents chat --background \
  --task-id 20802ea3-832d-4fb4-86f0-666ad79fcc80
```
...
coapis agents chat --background --task-id <id>
```

**Method 2: Timed polling** (if you must wait)
```bash
# Incrementally increasing intervals, fast first then slow
sleep 10 && coapis agents chat --background --task-id <id>
sleep 20 && coapis agents chat --background --task-id <id>
sleep 30 && coapis agents chat --background --task-id <id>
```
...
```bash
coapis agents -h
coapis agents list -h
coapis agents chat -h
```

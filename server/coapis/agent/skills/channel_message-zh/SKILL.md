...
description: 当需要主动向用户、会话或频道单向发送消息时，使用本 skill。通常仅在用户明确要求向某个 channel / 会话发送消息，或需要主动通知时使用。先用 coapis chats list 查询 session，再用 coapis channels send 推送消息。
metadata:
...
### 不应使用
- 如果只是当前会话中的正常回复，**不要使用 `coapis channels send`**
- 需要和用户进行双向对话并立即等待回复
- 还不知道目标 session 是哪个
...
2. coapis chats list 查询目标 session
3. 从结果中获取 user_id 和 session_id
4. 若有多个 session，优先选最近活跃的
5. coapis channels send 发送消息
6. 结束（无回复）
```
...
`coapis channels send` 只负责发送，不等待回复。
...
如果你正在当前会话里直接回复用户，不要使用 `coapis channels send`。
...
- **coapis agents chat**：发给其他 agent，双向，有回复
- **coapis channels send**：发给用户/会话/频道，单向，无回复

**选择原则**：
- 要找其他 agent 协作 → `coapis agents chat`
- 要主动给用户推送消息 → `coapis channels send`
...
- `--target-user`：目标用户ID（从 `coapis chats list` 获取）
- `--target-session`：目标会话ID（从 `coapis chats list` 获取）
- `--text`：消息内容

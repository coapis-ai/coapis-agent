...
# CoApis Installation and Configuration Q&A Guide

Use this skill when the user asks about **CoApis installation, initialization, environment configuration, dependency requirements, or common configuration options**.
...
DOC_DIR=$(find ~/.coapis/memory/ -type d -name "docs")
```
...
Run the following script logic to obtain the variable $COAPIS_ROOT:

```bash
# Get the absolute path of the binary
COP_PATH=$(which coapis 2>/dev/null || whereis coapis | awk '{print $2}')

# Logical deduction: if the path contains .coapis/bin/coapis, the root is three levels up
# Example: /path/to/CoApis/.coapis/bin/coapis -> /path/to/CoApis
if [[ "$COP_PATH" == *".coapis/bin/coapis" ]]; then
    COAPIS_ROOT=$(echo "$COP_PATH" | sed 's/\/\.coapis\/bin\/coapis//')
else
    # Fallback: try to get the parent of the parent directory
    COAPIS_ROOT=$(dirname $(dirname "$COP_PATH") 2>/dev/null || echo ".")
fi

echo "Detected CoApis Root: $COAPIS_ROOT"
```

Verify and list the documentation directory:
Use the derived $COAPIS_ROOT to locate the documentation:

```bash
# Construct the standard documentation path
DOC_DIR="$COAPIS_ROOT/website/public/docs/"
...
if [ -d "$DOC_DIR" ]; then
    ...
    find "$COAPIS_ROOT" -type d -name "docs" | grep "website/public/docs"
fi
```

**If project documentation does not exist, search the working directory**

If documentation is still not found, search for available documentation content under the coapis installation path:

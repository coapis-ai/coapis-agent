...
async def _ensure_browser(
    ...
    # Auto-connect to external CDP browser (e.g. docker-compose Playwright service)
    # when BROWSER_CDP_URL env var is set and no browser is running yet.
    if not state.get("connected_via_cdp") and not state.get("context"):
        import os
        env_cdp_url = os.environ.get("BROWSER_CDP_URL", "").strip()
        if env_cdp_url:
            logger.info(
                "BROWSER_CDP_URL detected (%s), auto-connecting...",
                env_cdp_url,
            )
            try:
                async_playwright = _ensure_playwright_async()
                ...
                if not state["pages"]:
                    ...
                    _register_page(state, page, page_id)
                    state["current_page_id"] = page_id
                logger.info("Connected to external CDP browser at %s", env_cdp_url)
            except Exception as e:
                logger.warning("Auto-connect to BROWSER_CDP_URL failed: %s", e)

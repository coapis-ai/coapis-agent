...
def _github_cache_ttl() -> float:
    raw = EnvVarLoader.get_str("COAPIS_GITHUB_CACHE_TTL", "")
    if raw:
        try:
...
def _hub_http_timeout() -> float:
    raw = EnvVarLoader.get_str("COAPIS_SKILLS_HUB_HTTP_TIMEOUT", "15")
    try:
...
def _hub_http_retries() -> int:
    raw = EnvVarLoader.get_str("COAPIS_SKILLS_HUB_HTTP_RETRIES", "3")
    try:
...
def _hub_http_backoff_base() -> float:
    raw = EnvVarLoader.get_str("COAPIS_SKILLS_HUB_HTTP_BACKOFF_BASE", "0.8")
    try:
...
def _hub_http_backoff_cap() -> float:
    raw = EnvVarLoader.get_str("COAPIS_SKILLS_HUB_HTTP_BACKOFF_CAP", "6")
    try:
...
def _hub_version_path() -> str:
    return EnvVarLoader.get_str(
        "COAPIS_SKILLS_HUB_VERSION_PATH",
...
def _build_request(full_url: str, accept: str) -> Request:
    req = Request(
        ...
        headers={
            "Accept": accept,
            "User-Agent": "coapis-skills-hub/1.0",
        },
    )

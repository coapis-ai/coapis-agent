...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repo-black.svg?logo=github)](https://github.com/agentscope-ai/CoApis)
[![PyPI](https://img.shields.io/pypi/v/coapis?color=3775A9&label=PyPI&logo=pypi)](https://pypi.org/project/coapis/)
[![Documentation](https://img.shields.io/badge/Docs-Website-green.svg?logo=readthedocs&label=Docs)](https://coapis.agentscope.io/)
...
[![Last Commit](https://img.shields.io/github/last-commit/agentscope-ai/CoApis)](https://github.com/agentscope-ai/CoApis)
[![License](https://img.shields.io/badge/license-Apache%202.0-red.svg?logo=apache&label=License)](LICENSE)
[![Code Style](https://img.shields.io/badge/code%20style-black-black.svg?logo=python&label=CodeStyle)](https://github.com/psf/black)
[![GitHub Stars](https://img.shields.io/github/stars/agentscope-ai/CoApis?style=flat&logo=github&color=yellow&label=Stars)](https://github.com/agentscope-ai/CoApis/stargazers)
[![GitHub Forks](https://img.shields.io/github/forks/agentscope-ai/CoApis?style=flat&logo=github&color=purple&label=Forks)](https://github.com/agentscope-ai/CoApis/network)
...
[[Documentation](https://coapis.agentscope.io/)] [[中文](README_zh.md)] [[日本語](README_ja.md)] [[Русский](README_ru.md)]

<p align="center">
  <img src="https://gw.alicdn.com/imgextra/i1/O1CN01sens5C1TuwioeGexL_!!6000000002443-55-tps-771-132.svg" alt="CoApis Logo" width="120">
</p>
...
>
> **Skills extension** — Built-in scheduling, PDF/Office processing, news digest, and more; custom skills auto-loaded, no lock-in. Skills determine what CoApis can do.
>
...
>
> **Every channel** — DingTalk, Feishu, WeChat, Discord, Telegram, and more. One CoApis, connect as needed.
>
...
>
> <details>
> <summary><b>What you can do with CoApis</b></summary>
>
> <br>
>
...
- [2026-04-24] We've released **v1.1.4**! See the full [v1.1.4 Release Notes](https://coapis.agentscope.io/release-notes).
...
- [2026-04-22] We've released **v1.1.3**! See the full [v1.1.3 Release Notes](https://coapis.agentscope.io/release-notes).

- [2026-04-17] We've released **v1.1.2**! See the full [v1.1.2 Release Notes](https://coapis.agentscope.io/release-notes).

- [2026-04-14] We've released **v1.1.1**! See the full [v1.1.1 Release Notes](https://coapis.agentscope.io/release-notes).

[2026-04-12] **CoApis is Officially Rebranding to CoApis**: This rebranding marks an important step forward into our next phase of open-source development.
...
From today on, CoApis will stay and grow with you!
...
> - **💬 I want to use in DingTalk/Feishu/WeChat**: Complete Quick Start → [Configure model](#api-key) → [Channel setup](https://coapis.agentscope.io/docs/channels)
> - **🐍 I don't want to install Python**: [Desktop app](#option-6-desktop-application-beta) or [Script install](#option-2-script-install) or [ModelScope Studio](https://modelscope.cn/studios/fork?target=AgentScope/CoApis)
...
```bash
pip install coapis
coapis init --defaults
coapis app
```

Then open the Console in your browser at **http://127.0.0.1:8088/** to configure your model. To chat in DingTalk, Feishu, WeChat, etc., see the [Channel setup](https://coapis.agentscope.io/docs/channels) documentation.
...
No Python setup required, one command installs everything. The script will automatically download uv (Python package manager), create a virtual environment, and install CoApis with all dependencies (including Node.js and frontend assets). Note: May not work in restricted network environments or corporate firewalls.

**macOS / Linux:**

```bash
curl -fsSL https://coapis.agentscope.io/install.sh | bash
```

**Windows (CMD):**

```CMD
curl -fsSL https://coapis.agentscope.io/install.bat -o install.bat && install.bat
```

**Windows (PowerShell):**

```powershell
irm https://coapis.agentscope.io/install.ps1 | iex
```
...
>      - Check if `uv` is available: Enter `uv --version` in CMD. If a version number appears, **only configure the CoApis path**. If you receive the prompt `'uv' is not recognized as an internal or external command, operable program or batch file,` configure both paths.
...
>      - CoApis path: Typically located at `%USERPROFILE%\.coapis\bin`.
...
>   - **Configure `uv` environment variables**: Add the `uv` directory and `%USERPROFILE%\.coapis\bin` to your system's `Path` variable.
>   - **Re-run the installation**: Open a new terminal and execute the installation script again to complete the `CoApis` installation.
>   - **Configure the `CoApis` environment variable**: Add `%USERPROFILE%\.coapis\bin` to your system's `Path` variable.

Once installed, open a new terminal and run:

```bash
coapis init --defaults   # or: coapis init (interactive)
coapis app
```
...
# Uninstall
coapis uninstall          # keeps config and data
coapis uninstall --purge  # removes everything
```
...
Images are on **Docker Hub** (`agentscope/coapis`). Image tags: `latest` (stable); `pre` (PyPI pre-release).

```bash
docker pull agentscope/coapis:latest
docker run -p 127.0.0.1:8088:8088 \
  -v coapis-data:/app/working \
  -v coapis-secrets:/app/working.secret \
  -v coapis-backups:/app/working.backups \
  agentscope/coapis:latest
```

Also available on Alibaba Cloud Container Registry (ACR) for users in China: `agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/coapis` (same tags).

Then open **http://127.0.0.1:8088/** for the Console. Config, memory, and skills are stored in the `coapis-data` volume; model provider settings and API keys are in the `coapis-secrets` volume; backup archives are stored in the `coapis-backups` volume. To pass API keys (e.g. `DASHSCOPE_API_KEY`), add `-e VAR=value` or `--env-file .env` to `docker run`.

> **Connecting to Ollama or other services on the host machine**
>
> Inside a Docker container, `localhost` refers to the container itself, not your host machine. If you run Ollama (or other model services) on the host and want CoApis in Docker to reach them, use one of these approaches:
>
...
>   agentscope/coapis:latest
> ```
> Then in CoApis **Settings → Models**, change the Base URL to `http://host.docker.internal:<port>` — for example, `http://host.docker.internal:11434` for Ollama, or `http://host.docker.internal:1234/v1` for LM Studio.
>
...
To run CoApis on Alibaba Cloud (ECS), use the one-click deployment: open the [CoApis on Alibaba Cloud (ECS) deployment link](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) and follow the prompts. For step-by-step instructions, see [Alibaba Cloud Developer: Deploy your AI assistant in 3 minutes](https://developer.aliyun.com/article/1713682).

---

### Option 5: Using ModelScope

**No local install?** [ModelScope Studio](https://modelscope.cn/studios/fork?target=AgentScope/CoApis) one-click cloud setup. Set your Studio to **non-public** so others cannot control your CoApis.
...
If you're not comfortable with command-line tools, you can download and use CoApis's desktop application without manually configuring Python environments or running commands.

#### Download

Download the desktop app from [GitHub Releases](https://github.com/agentscope-ai/CoApis/releases):
- **Windows**: `CoApis-Setup-<version>.exe`
- **macOS**: `CoApis-<version>-macOS.zip` (Apple Silicon recommended)
...
When you download the CoApis macOS app from Releases, macOS may show: *"Apple cannot verify that 'CoApis' contains no malicious software"*. This happens because the app is not notarized. You can still open it as follows:

- **Right-click to open (recommended)**
  Right-click (or Control+click) the CoApis app → **Open** → in the dialog click **Open** again. This tells Gatekeeper you trust the app; after that you can double-click to launch as usual.

- **Allow in System Settings**
  If it is still blocked, go to **System Settings → Privacy & Security**, scroll to the message like *"CoApis was blocked because it is from an unidentified developer"*, and click **Open Anyway** or **Allow**.

- **Remove quarantine attribute (not recommended for most users)**
  In Terminal run:
  `xattr -cr /Applications/CoApis.app`
...
For detailed usage instructions, troubleshooting, and common issues, see the [Desktop Application Guide](https://coapis.agentscope.io/docs/desktop).

---

## API Key

If you use a **cloud LLM API** (e.g., Qianwen, Gemini, OpenAI), you must configure an API key before chatting. CoApis will not work until a valid key is set. See the [official docs](https://coapis.agentscope.io/docs/models) for details.

**How to configure:**

1. **Console (recommended)** — After running `coapis app`, open **http://127.0.0.1:8088/** → **Settings** → **Models**. Choose a provider, enter the **API Key**, and enable that provider and model.
2. **`coapis init`** — When you run `coapis init`, it will guide you through configuring the LLM provider and API key. Follow the prompts to choose a provider and enter your key.
...
Tools that need extra keys (e.g. `TAVILY_API_KEY` for web search) can be set in Console **Settings → Environment variables**, see [Config](https://coapis.agentscope.io/docs/config) for details.

> **Using local models only?** If you use [Local Models](#local-models) (llama.cpp / Ollama / LM Studio), you do **not** need any API key.

## Local Models

CoApis can run LLMs entirely on your machine — no API keys or cloud services required. See the [official docs](https://coapis.agentscope.io/docs/models) for details.
...
| Topic                                                                 | Description                                      |
| --------------------------------------------------------------------- | ------------------------------------------------ |
| [Introduction](https://coapis.agentscope.io/docs/intro)                | What CoApis is and how to use it                  |
| [Quick start](https://coapis.agentscope.io/docs/quickstart)            | Install and run (local or ModelScope Studio)    |
| [Console](https://coapis.agentscope.io/docs/console)                   | Web UI: chat and agent configuration            |
| [Models](https://coapis.agentscope.io/docs/models)                     | Configure cloud, local, and custom providers    |
| [Channels](https://coapis.agentscope.io/docs/channels)                  | DingTalk, Feishu, QQ, Discord, iMessage, and more |
| [Skills](https://coapis.agentscope.io/docs/skills)                      | Extend and customize capabilities               |
| [Plugins](https://coapis.agentscope.io/docs/plugins)                    | Plugin system                                    |
| [MCP](https://coapis.agentscope.io/docs/mcp)                            | Manage MCP clients                               |
| [Memory](https://coapis.agentscope.io/docs/memory)                     | Long-term memory                     |
| [Memory-Evolving & Proactive](https://coapis.agentscope.io/docs/memory-evolving-and-proactive) | Agent memory evolution and proactive interaction |
| [Context](https://coapis.agentscope.io/docs/context)                   | Context management mechanism                     |
| [Magic commands](https://coapis.agentscope.io/docs/commands)           | Control conversation state without waiting for the AI |
| [Heartbeat](https://coapis.agentscope.io/docs/heartbeat)                | Scheduled check-in and digest                    |
| [Multi-Agent](https://coapis.agentscope.io/docs/multi-agent)           | Create multiple agents and enable collaboration  |
| [Config & working dir](https://coapis.agentscope.io/docs/config) | Working directory and config file                |
| [CLI](https://coapis.agentscope.io/docs/cli)                            | Init, cron jobs, skills, clean                   |
| [FAQ](https://coapis.agentscope.io/docs/faq)                           | Common questions and troubleshooting             |
...
CoApis includes multi-layer security mechanisms to protect your data and system:
...
- **Web Authentication** — Optional login protection for the Console. Disabled by default; set `COAPIS_AUTH_ENABLED=true` to enable. See [Web Authentication](https://coapis.agentscope.io/docs/security#Web-Authentication) for details.

See [Security documentation](https://coapis.agentscope.io/docs/security) for details.

---

## FAQ

For common questions, troubleshooting tips, and known issues, please visit the **[FAQ page](https://coapis.agentscope.io/docs/faq)**.

---

## Staying ahead

<a href="https://github.com/agentscope-ai/CoApis">
  <img src="https://img.alicdn.com/imgextra/i1/O1CN01V8HYv61By0HYcIDaq_!!6000000000013-1-tps-1698-954.gif" width="600" alt="Star CoApis" />
</a>

Star CoApis on GitHub and be instantly notified of new releases.
...
| **CoApis Custom Models**             | Multimodal model support                                                                                                                           | Planned              |
...
|                                       | Agent protocol: CoApis → CoApis                                                                                                                | In Progress          |
|                                       | Agent protocol: OpenClaw → CoApis                                                                                                               | Planned              |
...
```bash
git clone https://github.com/agentscope-ai/CoApis.git
cd CoApis
...
mkdir -p src/coapis/console
cp -R console/dist/. src/coapis/console/
...
- **Then**: Run `coapis init --defaults`, then `coapis app`.

> **Note for updates:** When updating to a new major version after `git pull`, please also rebuild the frontend, reinstall the package (`pip install -e .`), restart `coapis app`, and clear your browser cache with `Ctrl+Shift+R` (or `Cmd+Shift+R` on macOS).

---

## Contributing

CoApis evolves through open collaboration, and we welcome all forms of contribution! Check the [Roadmap](#roadmap) above (especially items marked **Seeking Contributors**) to find areas that interest you, and read [CONTRIBUTING](https://github.com/agentscope-ai/CoApis/blob/main/CONTRIBUTING.md) to get started. We particularly welcome:
...
Join [GitHub Discussions](https://github.com/agentscope-ai/CoApis/discussions) to discuss ideas or pick up tasks.

---

## Why CoApis?

CoApis stands for Qwen Personal Agent Workstation, and also embodies the wisdom of Qwen and the warmth of a Paw.
...
CoApis collects **anonymous** usage data during `coapis init` to help us understand our user base and prioritize improvements. Data is sent **once per version** — when you upgrade CoApis, telemetry is re-collected so we can track version adoption.

**What we collect:**

- CoApis version (e.g., 0.0.7)
...
When running `coapis init` interactively, you will be asked whether to opt in. If you choose `--defaults`, telemetry is accepted automatically. The prompt appears once per version and never affects CoApis's functionality.

---

## License

CoApis is released under the [Apache License 2.0](LICENSE).
...
<a href="https://github.com/agentscope-ai/CoApis/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=agentscope-ai/CoApis" alt="Contributors" />
</a>

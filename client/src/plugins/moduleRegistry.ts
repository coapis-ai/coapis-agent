 ...
 * 2. Plugins access and modify module exports via window.CoApis.modules
...
class ModuleRegistryImpl implements ModuleRegistry {
  ...
  /**
   * Get all modules (for window.CoApis.modules)
   */
...
// Expose to window.CoApis.modules (for plugin use)
// Set during initialization
if (typeof window !== "undefined") {
  if (!window.CoApis) {
    (window as any).CoApis = {};
  }

  // Use Proxy for dynamic access, ensuring plugins always get latest module state
  Object.defineProperty(window.CoApis, "modules", {
    get() {

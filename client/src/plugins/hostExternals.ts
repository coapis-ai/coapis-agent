 ...
 * `window.CoApis` so plugin bundles can register routes and tool renderers
...
/** Shared host dependencies exposed to plugin bundles via `window.CoApis.host`. */
...
export function installHostExternals(): void {
  ...
  if (!window.CoApis) {
    (window as any).CoApis = {} as WindowNamespace;
  }

  if (!window.CoApis.host) {
    window.CoApis.host = {
      React,
      ReactDOM,
      antd,
  ...
  if (!window.CoApis.registerRoutes) {
    window.CoApis.registerRoutes = (pluginId, routes) => {

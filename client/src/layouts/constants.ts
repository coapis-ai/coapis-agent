...
// export const PYPI_URL = "https://pypi.org/pypi/coapis/json";

export const GITHUB_URL = "https://github.com/coapis/coapis" as const;
...
export const getDocsUrl = (lang: string): string =>
  `https://coapis.dev/docs/intro?lang=${getWebsiteLang(lang)}`;

export const getFaqUrl = (lang: string): string =>
  `https://coapis.dev/docs/faq?lang=${getWebsiteLang(lang)}`;

export const getReleaseNotesUrl = (lang: string): string =>
  `https://coapis.dev/release-notes?lang=${getWebsiteLang(lang)}`;
...
要更新 CoApis 到最新版本，可根据你的安装方式选择对应方法：
...
cp -R client/dist/. server/console/
pip install -e .
\`\`\`
...
\`\`\`
docker pull coapis/coapis:latest
docker run -p 127.0.0.1:8088:8088 -v coapis-data:/app/working coapis/coapis:latest
\`\`\`

升级后重启服务 coapis app。`,

  ru: `### Как обновить CoApis

Чтобы обновить CoApis, выберите способ в зависимости от типа установки:
...
After upgrading, restart the service with \`coapis app\`.`,

  en: `### How to update CoApis

To update CoApis, use the method matching your installation type:

...
function renderDataTable(block: C2ABlock): React.ReactNode {
  ...
  const columns = headers.map((header, idx) => ({
    ...
    render: (_text: unknown, _record: Record<string, unknown>) => {
      ...
      // 检查该行是否有行级 action
      const actionKeys = Object.keys(rowActions);
      if (actionKeys.length > 0) {
        const actionKey = actionKeys[0];
        const action = rowActions[actionKey];
        if (action?.url_template && action.style === 'link') {
  ...
  if (Object.keys(rowActions).length === 0) {
    ...
    return <Table columns={columns} dataSource={tableData} pagination={false} size="small" bordered />;
  }

  // 有行级 action，在表格下方渲染操作按钮
  const tableData = rows.map((row, idx) => {
    const obj: Record<string, unknown> = { key: idx };
    row.forEach((cell, cellIdx) => {
      obj[`col_${cellIdx}`] = cell;
